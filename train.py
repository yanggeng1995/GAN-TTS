...
def train(args):
    ...
    g_criterion = HingeLoss().to(device)
    ...
    for epoch in range(args.epochs):
       ...
       for batch, (samples, conditions) in enumerate(train_data_loader):
            ...
            if num_gpu > 1:
                ...
                d_outputs = parallel(discriminator, (g_outputs, conditions))
            else:
                g_outputs = generator(conditions, z)
                d_outputs = discriminator(g_outputs, conditions)

            target = torch.ones_like(d_outputs).to(device)
            g_loss = g_criterion(g_outputs, samples) + d_criterion(d_outputs, target)
            ...
            #train discriminator
            if num_gpu > 1:
                fake_outputs = parallel(discriminator, (g_outputs, conditions))
                real_outputs = parallel(discriminator, (samples, conditions))
            else:
                fake_outputs = discriminator(g_outputs, conditions)
                real_outputs = discriminator(samples, condition)
                
            fake_target = torch.zeros_like(fake_outputs).to(device)
            real_target = torch.ones_like(real_outputs).to(device)
            fake_loss = d_criterion(fake_outputs, fake_target)
            real_loss = d_criterion(real_outputs, real_target)
...
def main():
    ...
    parser.add_argument('--condition_window', type=int, default=300)
